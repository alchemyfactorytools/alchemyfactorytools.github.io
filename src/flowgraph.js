...
// Pipes are effectively uncapped in-game (a single pipe carries 18000+/min; the only real
// limit is pipe length / attached reservoirs, which we don't model), so tile blueprints
// apply NO transport floor to liquid lines — they tile by machine load alone.
...
function buildFlowGraph(result, model, demand, opts = {}) {
  ...
  const summary = {
    ...
    liquidItems: Object.keys(model.db.items).filter((k) => model.db.items[k].liquid), // piped → belt-exempt
